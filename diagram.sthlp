...
{phang}
{cmd:diagram} {hline 2} {bf:diagram} generates dynamic diagrams using {bf:DOT} markup language and exports images in {bf:pdf}, {bf:png}, {bf:jpeg}, {bf:gif}, and {bf:bmp} format. For more information  {browse "http://www.haghish.com/dot":visit diagram homepage}.
...
{p 4 4 2}
{browse "http://www.graphviz.org/":graphViz} has several engines which are {bf:dot},
...
{p 4 4 2}
{browse "http://graphviz.org/pdf/dot.1.pdf":dot} - "directed graphs" which is the
default engine for rendering graphs where edges have directionality e.g.
{bf:A -> B}.

{p 4 4 2}
{browse "http://graphviz.org/pdf/neato.1.pdf":neato} is recommended for undirected diagrams,
especially when the size of the diagram is about 100 nodes or less.

{p 4 4 2}
{browse "http://graphviz.org/pdf/fdp.1.pdf":fdp} draws undirected graphs similar to
{bf:neato}, but applies different layouts.

{p 4 4 2}
{browse "http://graphviz.org/pdf/twopi.1.pdf":twopi} applies radial layouts.

{p 4 4 2}
{browse "http://graphviz.org/pdf/circo.1.pdf":circo} applies circular layouts.
